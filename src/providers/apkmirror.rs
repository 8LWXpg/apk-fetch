...
mod tests {
	...
	fn resolves_from_fixtures() {
		for (app, pkg) in APPS {
			let dir = root("apkmirror").join(app);
			let p = ApkMirror {
			...
			assert!(
				record.target.url.contains("download.php") || record.target.url.contains("downloadr"),
				"{app}: {}",
				record.target.url
			);
			assert!(
				record.target.headers.iter().any(|(k, _)| k == "Referer"),
				"{app}: no Referer"
			);
		}
	}
